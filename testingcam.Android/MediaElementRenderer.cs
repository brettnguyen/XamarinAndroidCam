...
using ToolKitMediaElement = testingcam.MediaElement2.Views.MediaElement2; 
using ToolKitMediaElementRenderer = testingcam.Droid.MediaElementRenderer;
using XCT = testingcam.Core;
...
namespace testingcam.Droid
{
    public class MediaElementRenderer : FrameLayout, IVisualElementRenderer, IViewRenderer, MediaPlayer.IOnCompletionListener, MediaPlayer.IOnInfoListener, MediaPlayer.IOnPreparedListener, MediaPlayer.IOnErrorListener
    {
       ...
       Surface texture;
        ...
        public virtual void UnsubscribeFromEvents(ToolKitMediaElement element)
        {
            if (element == null)
                return;
        ...
        void StateRequested(object? sender, StateRequested e)
        {
            ...
            switch (e.State)
            {
                case MediaElement2State.Playing:
                    view.Start();
                    Controller.CurrentState = view.IsPlaying ? MediaElement2State.Playing : MediaElement2State.Stopped;
                    break;

                case MediaElement2State.Paused:
                    if (view.CanPause())
                    {
                        view.Pause();
                        Controller.CurrentState = MediaElement2State.Paused;
                    }
                    break;

                case MediaElement2State.Stopped:
                    view.Pause();
                    view.SeekTo(0);
                    Controller.CurrentState = view.IsPlaying ? MediaElement2State.Playing : MediaElement2State.Stopped;
                    break;
        ...
        public void SeekRequested(object? sender, SeekRequested e)
        {
            if (view == null || Controller == null)
                return;
        ...
        protected virtual void UpdateSource()
        {
            ...
            else
            {
                ...
                if (Controller != null)
                    Controller.CurrentState = MediaElement2State.Stopped;
            }
        }
        ...
        void MediaPlayer.IOnPreparedListener.OnPrepared(MediaPlayer? mp)
        {
            ...
            {
                mediaPlayer.Start();
                Controller.CurrentState = MediaElement2State.Playing;
            }
            else
                Controller.CurrentState = MediaElement2State.Paused;
        }
        ...
        bool MediaPlayer.IOnInfoListener.OnInfo(MediaPlayer? mp, MediaInfo what, int extra)
        {
            ...
            switch (what)
            {
                case MediaInfo.BufferingStart:
                    Controller.CurrentState = MediaElement2State.Buffering;
